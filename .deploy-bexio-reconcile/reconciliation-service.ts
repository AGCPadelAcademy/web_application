...
async function processRetries(deps: ReconcileDeps, now: Date, counts: ReconcileResult): Promise<void> {
  ...
  for (const op of due) {
    try {
      ...
      if (
        err instanceof BookingNotBillableError ||
        err instanceof NoBillingDocumentError ||
        err instanceof RefundAgreementRequiredError ||
        err instanceof InvoiceCancelConflictError
      ) {
        await deps.repo.upsertOperation({
          ...op,
          status: 'failed',
