...
| ℹ️ INFO | `bookings_duplicate`, `invoice_counters`, `invoices` have RLS enabled and zero policies | Three service-only/legacy tables | **Intentional deny-by-default.** `0009` revokes `anon`/`authenticated` table privileges as defense in depth. Advisor INFO remains because no permissive policies are intentionally added. |
...
| ❗ ERROR | `booking_slots`, `billing_public_config`, `session_roster` are SECURITY DEFINER views | Three narrow projection views | **Accepted architecture, hardened by `0009`.** Definer rights are required to expose only a safe projection while underlying tables stay RLS-protected. API roles receive SELECT only; `security_barrier` is enabled. Replacing these requires a dedicated API redesign. [Docs](https://supabase.com/docs/guides/database/database-linter?lint=0010_security_definer_view) |
