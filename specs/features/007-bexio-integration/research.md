...
- **Decision**: Positions carry `tax_id` of Bexio tax **code VIM** (production, 2026-08-29). Invoice-level `mwst_type` 0 and `mwst_is_net=false` (advertised lesson price is the invoice total). Discovery lists the **full tax catalog** (`/3.0/taxes?limit=2000`, not only `types=sales_tax`) and selects `code === 'VIM'`; issuance prefers VIM from that list, then a 0% rate, then the stored id. If VIM (and no 0% fallback) is missing, `tax_id_sales` is incomplete. Price amounts come exclusively from AGC's `lesson_types.price`.
- **Rationale**: The academy's production invoices must not show 8.1% VAT. The sales-tax-only filter on the production company returns only UN81 8.1% / UR26 2.6%. Live catalog (2026-08-29): VIM is id 9, type `pre_tax_material`, value 0, active. The accountant-specified code is VIM. Selecting by code (then 0%) rather than `taxes[0]` avoids the demo-company ordering trap; the numeric id is still discovered from Bexio, not hardcoded in application source.
- **Alternatives considered**: *Using taxes[0]* — rejected 2026-08-29: that was 8.1% on the demo company. *Selecting only value === 0 among sales_tax* — rejected the same day: production has no 0% row in that filter (`tax_id_sales` stayed missing). *Hardcoding a tax id* — rejected: production ids differ from demo. *UEX (id 3, active `not_taxable_turnover` 0%) or U00 (id 8, inactive `sales_tax` 0%)* — not used unless Bexio rejects VIM on invoice positions (docs restrict `tax_id` to active sales taxes).
