...
- Q: Discover reported missing `tax_id_sales` after selecting by rate 0. Which Bexio tax should lesson invoices use? → A: **Code VIM** (superseded the same day — see next session). Production `GET /3.0/taxes?types=sales_tax&scope=active` only returns 8.1% (id 14 / UN81) and 2.6% (id 15 / UR26). Live catalog: VIM is id 9, type `pre_tax_material`.

### Session 2026-08-29 (production tax code UEX)

- Q: VIM is a purchase/import 0% code. Should lesson *sales* invoices use UEX instead? → A: **Yes. Code UEX.** Live catalog (`GET /3.0/taxes?limit=2000`, 2026-08-29): UEX is id **3**, type `not_taxable_turnover`, value 0, active, display name `UEX - Export/Exempt 0.00%`. Bexio invoice positions do not accept `pre_tax_material` (VIM). Discovery lists the full tax catalog and selects `code === 'UEX'`; issuance prefers UEX from `taxes_sales`, then a 0% rate, then the stored id. Never `taxes[0]`, never VIM.
