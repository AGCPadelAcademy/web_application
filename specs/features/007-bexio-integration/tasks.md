...
- [X] T003 [P] Prepare the Bexio demo company per quickstart.md §0.2: QR-capable bank account, invoice template with QR payment part, automatic invoice numbering, active sales tax (FR-018 prerequisites). **DONE 2026-08-21** (VAT confirmed: standard rate 8.1%, net prices; demo company tax id 14)
...
- [X] T017 [US1] Deploy `bexio-oauth` and verify quickstart.md §1.1–§1.4 end-to-end against the demo company. **DONE 2026-08-21**: live OAuth connect succeeded end-to-end (status `connected`, tokens in Vault, scopes verified); config discovery completed against the demo company (`currency_id=1`, tax 8.1% id 14, bank account, payment type, unit, language, country). Fixes applied along the way: IdP migration to `auth.bexio.com`, path-based callback, gateway prefix-stripping in callback routing, `/3.0/currencies` `name`-field matching
...
- [X] T038 [US4] Deploy `bexio-reconcile`, confirm the `pg_cron` job from T010 invokes it, and verify quickstart.md §4 on the test branch. **DONE 2026-08-25**: live payment + **Run reconciliation now** confirmed the booking. Automatic 15-min tick is the remaining observational check.
...
**Independent Test**: quickstart.md §8 — unpaid cancel from My Payments; paid lesson has no cancel; admin Integrations has no cancel-invoice UI
...
- [X] T043 [US5] Deploy `billing-cancel-invoice` v1 and `bexio-reconcile` v2 (2026-08-25). Remaining live check: student unpaid cancel from My Payments (quickstart §8).
...
- [ ] T045 [US6] Deferred
- [ ] T046 [P] [US6] Deferred
- [ ] T047 [US6] Deferred — quickstart §9 removed
...
- **US6 (P3)**: Deferred 2026-08-25 — clients see invoice status on My Payments; no admin ledger in 007
...
- US6: deferred (no T044–T047 work in 007)
- Polish: T049 ∥ T050
...
6. + US5 → client invoice status + unpaid lesson cancel → validate quickstart §7; US6 is deferred
