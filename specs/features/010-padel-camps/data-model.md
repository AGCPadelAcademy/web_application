...
**Flyer storage (2026-09-09, C2)**: private `camp-flyers` bucket; object path `{camp_id}/{file}`; images ≤ 5 MB. `storage.objects` policies: `SELECT` for anon/authenticated when the first path segment is a **published** Camp id, or the caller is an admin; `INSERT/UPDATE/DELETE` admin-only. The SPA reads flyers through short-lived signed URLs. Unpublished Camps' flyers are never publicly readable (convergence-2 §4). **(Amended 2026-09-09 C4, 0019)** The SELECT policy MUST reference `storage.objects.name` (the object path). Unqualified `name` inside `EXISTS (SELECT … FROM camps)` binds to `camps.name` and makes anon signed URLs 404.
...
3. Count active registrations (`pending_payment`,`confirmed`). If `count >= camp_registration_ceiling(...)` → raise `camp_full`. **(Amended 2026-09-24 C9)** Junior (`camp_type`) ceiling is `max_capacity` until 3 places remain, then `max_capacity + 6`. `camp_places_remaining` shows 3 through 13 active, 2 at 14, 1 at 15, and 0 at 16. Other types stay on `max_capacity`.
