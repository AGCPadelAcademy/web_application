...
| 0017 flyers + dual pricing apply (2026-09-09) | PASS — applied to production on explicit user request; 11/11 assertions pass (the member-price CHECK assertion uses `%member_price_amount >=%` because Postgres renders the bound as `(0)::numeric`). `camp-submit-registration` and `camp-admin` redeployed (claim passthrough; member-price validation + claim flag in registrations) |
