...
### Session 2026-09-10 (round 6 — register child, extras, flyer chrome)

- Q: Should `+ Add new Child` return on Camp register? → A: **Yes.** On the Submit registration card it navigates to My Children so the parent can create the child, then returns to this Camp with the draft (selected extras, membership, terms, child) intact. Child create stays on the Children page (no inline mini-form). FR-012 stays: several children still means several separate registrations. This **supersedes** the round-3 removal.
- Q: Where is an extra’s description shown? → A: Next to the extra name and price on the Submit registration card, and on the admin Camp form extras list. Empty description is omitted. Admins can edit an extra’s name, price, and description (not add-only).
...
### Session 2026-09-24 (round 8 — Junior dynamic capacity)
...
6. **(Added 2026-09-10 C6)** **Given** extras on a Camp, **When** the admin views the Camp form extras list, **Then** each extra shows name, price, and description (when set). **When** they edit an extra, **Then** name, price, and description can be changed and saved without deleting and re-adding it.
...
1. **Given** published Camps, **When** a visitor opens `/camps`, **Then** they see each published Camp’s name, dates, schedule/hours, eligibility/ages, description, registration status/deadline, and availability where it is appropriate to show — **not** the usual or academy-member price. **(Amended 2026-09-09 C3)** Each Camp occupies its own row; the flyer (when present) is large and keep-aspect; the page description matches the card-container width. **(Amended 2026-09-09 C4)** On a mobile-width viewport the flyer remains visible and tappable. **(Amended 2026-09-09 C5)** Camps are listed by start date (earlier week first), and within a week Mini then Junior then Competition. **(Amended 2026-09-10 C6)** The flyer control is sized to the flyer (no oversized empty frame). The larger view is the flyer only (blurred background, close on the image, click-outside/Escape).
...
- **FR-006b**: The same child records MUST be reusable for future Camps. Camp registration MUST select an existing child from the parent’s list; adding a new child happens on the Children page (FR-006a), not as an inline mini-form on registration. **(Amended 2026-09-10 C6)** The Submit registration card MUST offer `+ Add new Child`, which navigates to My Children and returns to this Camp with the registration draft (selected extras, membership, terms) intact and the new child selected. It MUST NOT create a throwaway child that cannot be reused.
...
- **FR-016**: A Camp MAY define zero or more extras, each with name, description, price, and active/configured state. Lunch is an example only and MUST NOT be hardcoded. **(Amended 2026-09-10 C6)** The Submit registration card and the admin Camp form extras list MUST show the extra description next to name and price when it is set. Admins MUST be able to edit an extra’s name, price, and description.
