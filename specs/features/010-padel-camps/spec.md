...
2. **Given** a parent with no saved children yet, **When** they start registration, **Then** they can add a child (same personal information as the Children page) and that child is stored on their list for future Camps as well as used for this registration. **(Amended 2026-09-09)** The registration form offers a `+ Add new Child` action that routes to the create-child form and returns with the registration draft restored (FR-008a) and the new child available for selection.
...
- **FR-008a**: When a parent opens Terms and Conditions from the registration flow, returning from the Terms page MUST navigate back to the page the terms were opened from — never the home page — and the registration draft (selected child, selected extras, terms state, and other entered registration data) MUST be preserved so the parent continues without re-entering anything. The `+ Add new Child` action on the registration form MUST route to the create-child form and, after creation, return to the registration with the same draft preserved and the newly created child available for selection.
