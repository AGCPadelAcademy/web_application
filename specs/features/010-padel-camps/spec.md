...
1. **Given** published Camps, **When** a visitor opens `/camps`, **Then** they see each published Camp’s name, dates, schedule/hours, eligibility/ages, description, registration status/deadline, and availability where it is appropriate to show — **not** the usual or academy-member price. **(Amended 2026-09-09 C3)** Each Camp occupies its own row; the flyer (when present) is large and keep-aspect; the page description matches the card-container width. **(Amended 2026-09-09 C4)** On a mobile-width viewport the flyer remains visible and tappable.
...
- Preserve lesson booking, F1.03 invoice/reconciliation for lesson bookings, and the existing My Payments lesson rows. **(Amended 2026-09-09 C4)** Extend My Payments to also list Camp registration invoices without folding Camps into `bookings`.
