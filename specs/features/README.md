...
| `011-membership-automatic-collection` | F1.26 — in-app Membership auto-pay via eBill Direct Debit mandate + F1.03 period invoice + payment-confirmed signal (QR fallback) |
...
6. When the feature ships, update the baseline files it actually changes (`requirements.md`, `domain-model.md`, `api-contracts.md`, inventory, etc.). This always includes `specs/baseline-system/supabase-backend.md` when the feature touched the backend (tables, views, RLS policies, Edge Functions, Storage, migrations, extensions): refresh the affected sections, row counts, and function versions **against the live project** (Supabase MCP / dashboard — do not document from the migration files alone), and add a dated `> Refreshed ...` line at the top. A feature's implementation is not complete until this sync is done.
7. New database objects follow the schema layout in `supabase-backend.md` §2: domain tables go in `public` with RLS enabled in the same migration; privileged fixed-output readers go in the unexposed `private` schema; do not create per-domain schemas, and never touch Supabase-managed schemas (`auth`, `storage`, `vault`, `cron`, `net`, `extensions`, ...).
