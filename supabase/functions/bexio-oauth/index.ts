...
import { deleteSecret, readSecret, writeSecret } from "../_shared/billing/vault.ts";
...
async function requireAdmin(req: Request): Promise<{ userId: string } | Response> {
  ...
  const rows = (await dbSelect("profiles", `id=eq.${user.id}&select=role,is_active`)) as {
    role: string;
    is_active: boolean;
  }[];
  if (!canAdminister(parseProfileAccess(rows[0]))) {
    return json({ error: "active admin role required" }, 403);
  }
...
async function handleStart(userId: string): Promise<Response> {
  if (!CLIENT_ID || !STATE_SECRET) {
    return json({ error: "integration not configured server-side" }, 500);
  }
...
async function handleCallback(url: URL): Promise<Response> {
  ...
  const profiles = (await dbSelect(
    "profiles",
    `id=eq.${check.userId}&select=role,is_active`,
  )) as { role: string; is_active: boolean }[];
  if (!canAdminister(parseProfileAccess(profiles[0]))) {
    return redirect("bexio=error&code=admin_inactive");
  }
...
async function handleInitialize(): Promise<Response> {
  ...
  await discover(
    "currency_id",
    "/3.0/currencies",
    (rows: { id: number; name: string }[]) => rows.find((r) => r.name === "CHF")?.id ?? null,
  );
  await discover(
    "bank_account_id",
    "/3.0/banking/accounts",
    (rows: { id: number }[]) => rows[0]?.id ?? null,
  );
  await discover(
    "payment_type_id",
    "/2.0/payment_type",
  ...
  try {
    ...
    config.country_id_ch = countryRows.find((r) =>
      (r.iso3166_alpha2 || r.name_short) === "CH"
    )?.id ??
  ...
  await discover(
    "sales_account_id",
    "/2.0/accounts",
    (rows: { id: number; account_no: string; is_active: boolean }[]) =>
...
async function handleConfigure(body: { config?: Record<string, unknown> }): Promise<Response> {
  ...
  if (typeof patch.tax_id_sales === "number") {
    await check("tax_id_sales", `/3.0/taxes/${patch.tax_id_sales}`);
  }
  if (typeof patch.currency_id === "number") await check("currency_id", "/3.0/currencies");
  if (typeof patch.bank_account_id === "number") {
    await check("bank_account_id", "/3.0/banking/accounts");
  }
  if (typeof patch.payment_type_id === "number") {
    await check("payment_type_id", "/2.0/payment_type");
  }
