...
async function maybeEmailInvoicePdf(opts: {
  ...
  try {
    const already = await dbSelect(
      "notifications_log",
      `booking_id=eq.${opts.bookingId}&status=eq.sent&message_subject=like.${
    ...
    const profiles = await dbSelect(
      "profiles",
      `id=eq.${opts.userId}&select=email,full_name,first_name`,
    );
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (err instanceof ProviderAuthError || err instanceof ProviderUnavailableError) {
      ...
      return json(
        { error: "provider_unavailable", message: "invoice issuing queued for retry" },
        502,
      );
    }
