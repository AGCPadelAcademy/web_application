...
import { PageSizes, PDFDocument, rgb, StandardFonts } from "https://esm.sh/pdf-lib@1.17.1";
...
async function buildInvoicePage(
  ...
  y -= 15;
  if (customerAddress) {
    dt(customerAddress, M, y, 10, font, DARK_GRAY);
    y -= 13;
  }
  if (customerCity) {
    dt(customerCity, M, y, 10, font, DARK_GRAY);
    y -= 13;
  }
  if (customerCountry) {
    dt(customerCountry, M, y, 10, font, DARK_GRAY);
    y -= 13;
  }
  y -= 22;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      booking_id,
      invoice_number: incomingInvoiceNumber,
      amount,
      invoice_date,
      customer_fullname,
      customer_address,
    ...
    if (
      !canMutateOwnedResource(
        parseProfileAccess(callerProfile),
        bookingRow.user_id === user.id,
      )
    ) {
      return jsonResponse(
        { success: false, error: "Forbidden: inactive or unauthorized caller" },
        403,
      );
    }
    ...
    if (!invoiceNumber) {
      ...
      if (sequenceError || typeof sequence !== "number") {
        throw new Error(
          `Invoice number allocation failed: ${sequenceError?.message ?? "no sequence returned"}`,
        );
      }
    ...
    if (invoice_id) {
      ...
      const { data, error: insertError } = await supabase
        ...
        .single();
      if (insertError) {
        console.error("[generate-invoice-pdf] DB insert error:", insertError.message);
      }
      invoiceRecord = data;
    }
    if (pdfUrl) {
